...
const DFS_COMPLEXITY: TraceComplexity = {
  timeWorst: "O(R*C)",
  spaceWorst: "O(R*C)",
};
...
function parseGridDfsInput(input: GridDfsInput): ParsedInput {
...
function serializeInput(input: GridDfsInput): string {
  const parsed = parseGridDfsInput(input);
...
function parseInputText(text: string): GridDfsInput {
...
function snapshotFrom(state: MutableState): GridDfsSnapshot {
  return {
    rows: state.rows,
    cols: state.cols,
    start: state.start,
...
function makeStep(
  ...
  title: string,
  description: string,
  phase: TraceStep<GridDfsSnapshot>["phase"],
  state: MutableState,
  isError = false,
): TraceStep<GridDfsSnapshot> {
  return {
    id,
    title,
    description,
...
function runDfs(parsed: ParsedInput): TraceResult<GridDfsSnapshot> {
  ...
  const steps: Array<TraceStep<GridDfsSnapshot>> = [
    makeStep(
      "dfs-init",
      "초기화",
      "시작 노드를 stack에 넣고 DFS를 시작합니다.",
      "enter",
      state,
    ),
  ];
  ...
  while (state.stack.length > 0) {
    ...
    state.frontier = state.stack.slice();
    state.expanded += 1;
    ...
    steps.push(
      makeStep(
        `dfs-enter-${stepCount}`,
        "노드 확장",
        `(${current.row}, ${current.col})를 stack에서 꺼내 확장합니다.`,
        "enter",
        state,
      ),
    );
    ...
    if (currentKey === goalKey) {
      ...
      steps.push(
        makeStep(
          `dfs-exit-success-${stepCount}`,
          "탐색 성공",
          "목표 지점에 도달해 경로를 복원했습니다.",
          "exit",
          state,
        ),
      );
    ...
    for (let index = DIRECTIONS.length - 1; index >= 0; index -= 1) {
      ...
      steps.push(
        makeStep(
          `dfs-update-push-${stepCount}`,
          "이웃 추가",
          `(${nr}, ${nc})를 stack에 추가합니다.`,
          "update",
          state,
        ),
      );
  ...
  steps.push(
    makeStep(
      `dfs-exit-fail-${stepCount}`,
      "탐색 실패",
      "stack을 모두 소진했지만 목표를 찾지 못했습니다.",
      "exit",
      state,
      true,
    ),
...
export const gridDfsAdapter: ProblemTraceAdapter<GridDfsInput, GridDfsSnapshot> = {
  id: "grid-dfs",
  title: "Grid DFS Trace",
  description:
    "문제를 그래프(grid)로 보고 DFS 탐색/가지치기/경로 복원을 step 단위로 시각화합니다.",
  inputFields: [
    {
      key: "mapMode",
      label: "mapMode",
      ...
      return JSON.stringify(
        {
          ...
          walls: input.walls
            ...
            .map((token) => token.trim())
            .filter((token) => token.length > 0)
            .map((token) => token.split(",").map((value) => Number(value.trim()))),
        },
        null,
        2,
      );
  ...
  run: (input) => {
    try {
      return runDfs(parseGridDfsInput(input));
    } catch (error) {
      const message = error instanceof Error ? error.message : "입력 검증에 실패했습니다.";
      const fallback: GridDfsSnapshot = {
        rows: 1,
        cols: 1,
        start: { row: 0, col: 0 },
        goal: { row: 0, col: 0 },
