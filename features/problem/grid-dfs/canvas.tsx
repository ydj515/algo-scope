...
import type { GridDfsSnapshot } from "@/features/problem/grid-dfs/adapter";
import { Badge } from "@/features/ui/components/badge";
import { CanvasFrame } from "@/features/ui/components/canvas-frame";

type Props = {
  snapshot: GridDfsSnapshot | null;
};
...
export function GridDfsCanvas({ snapshot }: Props) {
  if (!snapshot) {
    return (
      <CanvasFrame
        hasData={false}
    ...
    <CanvasFrame
      ...
      header={(
        <div className="flex items-center justify-between gap-2">
          <p className="text-sm font-semibold text-[var(--color-fg)]">Grid DFS Canvas</p>
      ...
      legend={(
        <div className="flex flex-wrap gap-2">
          ...
          <Badge tone="info">frontier(stack)</Badge>
